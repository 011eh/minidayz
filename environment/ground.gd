...
func _ready():
	...
	var paths_block_dict := create_path_block(location_block)
...
func terr_get_path_block_type(axis: String, coords: Vector2i, paths: Dictionary, locations := [] as Array[Vector2i]) -> Vector2i:
	if paths.has(coords):
		return Vector2i(2, 2)
	else:
		var v: Vector2
...
class BlockData:
	...
	func _init(coords: Vector2i, type: Type):
		self.coords = coords
		self.type = type
